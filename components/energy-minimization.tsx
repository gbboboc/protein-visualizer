...
const EnergyMinimization: React.FC<EnergyMinimizationProps> = ({
  ...
  const [energyHistory, setEnergyHistory] = useState<{ iteration: number; energy: number }[]>([])
  ...
  const runMonteCarloSimulation = async () => {
    ...
    for (let i = 1; i <= iterations; i++) {
      ...
      newDirections[randomIndex] = possibleDirections[Math.floor(Math.random() * possibleDirections.length)]
      ...
      setEnergyHistory((prev) => [...prev, { iteration: i, energy: currentEnergy }])
  ...
  const runSimulatedAnnealing = async () => {
    ...
    for (let i = 1; i <= iterations; i++) {
      ...
      if (newEnergy < currentEnergy) {
        ...
        const acceptanceProbability = Math.exp((currentEnergy - newEnergy) / currentTemp)
        if (Math.random() < acceptanceProbability) {
          accepted = true
        }
      }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {isRunning && (
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>Progress</span>
                <span>{Math.round((currentIteration / iterations) * 100)}%</span>
              </div>
          ...
          <div className="pt-2">
            <Button onClick={handleStartOptimization} disabled={isRunning} className="w-full">
          ...
          <div className="grid grid-cols-2 gap-4 pt-2">
            <div className="bg-gray-50 p-3 rounded-md">
              <h3 className="text-sm font-medium text-gray-700">Current Energy</h3>
        ...
        <CardContent>
          <div className="h-[300px] bg-gray-50 rounded-md overflow-hidden">
            <Canvas>
              ...
              <ProteinModel sequence={sequence} directions={directions as any} visualizationType="3d" />
            </Canvas>
          </div>
