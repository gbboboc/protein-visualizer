...
const ExportOptions: React.FC<ExportOptionsProps> = ({
  ...
  const [fileName, setFileName] = useState<string>(`${proteinName.toLowerCase().replace(/\s+/g, "_")}`)
  ...
  const generatePDB = (): string => {
    ...
    for (let i = 0; i < sequence.length; i++) {
      ...
      pdb += `ATOM  ${(i + 1).toString().padStart(5, " ")}  CA  ${residueType} A${(i + 1).toString().padStart(4, " ")}    ${pos.x.toFixed(3).padStart(8, " ")}${pos.y.toFixed(3).padStart(8, " ")}${pos.z.toFixed(3).padStart(8, " ")}  1.00  0.00           C\n`
    }

    // Generate CONECT records for bonds
    for (let i = 0; i < sequence.length - 1; i++) {
      pdb += `CONECT${(i + 1).toString().padStart(5, " ")}${(i + 2).toString().padStart(5, " ")}\n`
    }
  ...
  const handleCopySequence = () => {
    ...
    toast({
      ...
      description: "Protein sequence has been copied to clipboard.",
    })
  }
        ...
        <Tabs defaultValue="file">
          ...
          <TabsContent value="file" className="space-y-4">
            ...
            <Button onClick={handleExport} className="w-full">
              <Download className="w-4 h-4 mr-2" /> Export {exportType.toUpperCase()}
            </Button>
          </TabsContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex mt-2">
                  <Input value={sequence} readOnly className="flex-1" />
                  <Button variant="outline" onClick={handleCopySequence} className="ml-2">
              ...
              {directions && directions.length > 0 && (
                <div>
                  <Label>Folding Directions</Label>
                  <div className="flex mt-2">
                    <Input value={directions.join("-")} readOnly className="flex-1" />
                    <Button
                      variant="outline"
                      onClick={() => {
                        navigator.clipboard.writeText(directions.join("-"))
                        toast({
                          title: "Copied to Clipboard",
                          description: "Folding directions have been copied to clipboard.",
                        })
                      }}
                      className="ml-2"
                    >
          ...
          <TabsContent value="share">
            <div className="space-y-4">
              <p className="text-sm text-gray-500">
                Generate a shareable link that contains the protein sequence and folding directions.
              </p>
